...
class DonkeyUnitySim(object):
    ...
    def observe(self):
        ...
        observation = {}
        observation["image"] = self.image_array
        observation["speed"] = self.speed
    ...
    def is_game_over(self):
        ...
        if abs(self.cte) > (self.CTE_MAX_ERR) or self.hit != "none" or self.stopped>=10: # + 0.2
            return True
        return False
    ...
    def calc_reward(self, done):
        ...
        #return ((1.0 - (abs(self.cte) / self.CTE_MAX_ERR))**2) * self.speed / 30
        return (1.0 - (abs(self.cte) / self.CTE_MAX_ERR)) * self.speed / 30
        #return (1.0 - (abs(self.cte) / self.CTE_MAX_ERR)) * math.tanh(self.speed / 10)
        #return 1 + math.tanh(self.speed / 10)


    ## ------ Launch Unity Env ----------- ##

    def executable_launcher(self, file_name, headless=False, platform="windows"):

        # TODO: Only for MACOS for now. Need to handle Linux
        cwd = os.getcwd()
        file_name2 = (file_name.strip().replace('.app', '').replace('.exe', '').replace('.x86_64', '').replace('.x86', ''))
    ...
    def send_control(self, steering_angle, throttle):
        ...
        if self.last_action[0] == steering_angle and self.last_action[1] == throttle:
            self.same_action = True
        else:
            self.same_action = False

        self.last_action[0] = steering_angle
        self.last_action[1] = throttle
