...
x_train = np.zeros((len_train, 40, 80))
...
for idx, f in enumerate(files):
    ...
    x_train[idx] = np.copy(im)
    x_train[idx] = x_train[idx] / 255
    if idx % 1000 == 0:
        print(idx)

x_train = x_train.reshape((len_train, 40, 80, 1))

'''
files = glob.glob("./test/*.jpg")
len_test = len(files)

x_test = np.zeros((len_test, 120, 160))

for idx, f in enumerate(files):
    im = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
    #im = cv2.GaussianBlur(im, (3,3), 0)
    #im = cv2.Canny(im, 100, 250)
    x_test[idx] = np.copy(im)
    x_test[idx] = x_test[idx] / 255
    if idx % 1000 == 0:
        print(idx)

x_test = x_test.reshape((len_test, 120, 160, 1))
'''
...
if __name__ == '__main__':
    ...
    if args.weights:
        ...
        for idx,_ in enumerate(x_train):
            plt.imshow(x_train[idx].reshape(40,80))
            plt.show()
            plt.imshow(predicted[idx].reshape(40,80))
            plt.show()
    else:
