...
if __name__ == '__main__':
    ...
    while agent.episode <= 300:
        ...
        state={}
        state["image"] = np.expand_dims(decoded_data, axis=0)
        state["speed"] = np.expand_dims(np.array([data["speed"]/30]), axis=0)
        ...
        while not done:
            ...
            if train:
                if not restart:
                    if agent.tick_episode >= 1:
                        agent.remember([previous_state["image"],previous_state["speed"]], action, reward, [state["image"],state["speed"]], done)
            ...
            if done:
                ...
                state["image"] = np.expand_dims(decoded_data, axis=0)
                state["speed"] = np.expand_dims(np.array([data["speed"]/30]), axis=0)
                previous_state = previous_state = copy.deepcopy(state)
