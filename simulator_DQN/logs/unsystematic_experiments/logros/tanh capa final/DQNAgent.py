...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        if self.model_type == 0:
            ...
            model.add(Dense(7, activation='tanh', kernel_initializer="lecun_normal"))
            ...
            conv = Flatten()(conv)
            advt = Dense(50)(conv)
            advt = LeakyReLU(alpha=0.1)(advt)
            advt = Dense(self.action_size)(advt)
            value = Dense(50)(conv)
            value = LeakyReLU(alpha=0.1)(value)
            value = Dense(1)(value)
    ...
    def train(self, tupla):
        ...
        if not done:
            if self.upgrade_mode == 0: #Normal
                ...
                target =  reward + self.gamma * math.atanh(np.clip(np.max(self.model.predict(next_state)[0]), -0.999, 0.999))
            elif self.upgrade_mode == 1: #Fixed q-target
        ...
        target_f[0][action] = math.tanh(target)
