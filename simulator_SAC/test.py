...
parser.add_argument('-n', '--n-timesteps', help='number of timesteps', default=1,
                    type=int)
...
print(log_path)
path_list = glob.glob(os.path.join(log_path, '{}_best_*.pkl'.format(ENV_ID)))
print(path_list)
#best_path = ''
#if args.best_model:
#    best_path = '_best'
...
for path in path_list:
    ...
    while episode < args.n_timesteps:
        for _ in range(3001):
            if not done:
                ...
                if (done or ep_len>=3000) and args.verbose >= 1:
    ...
    with open("{}\\test2.txt".format(log_path), "a") as info:
        info.write("{},{},{},{}\n".format(path, total_reward/args.n_timesteps, total_length/args.n_timesteps, passed_test))
        info.close()
    if passed_test < 3:
