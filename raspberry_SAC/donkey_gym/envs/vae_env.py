...
from config import INPUT_DIM, MIN_STEERING, MAX_STEERING, JERK_REWARD_WEIGHT, MAX_STEERING_DIFF, ROI, N_CHANNELS
...
class DonkeyVAEEnv(gym.Env):
    ...
    def __init__(self, level=0, frame_skip=2, vae=None, const_throttle=None,
                 ...
                 n_stack=1,
                 save_for_vae=False, save_obs=False):
        self.vae = vae
        self.z_size = None
        if vae is not None:
        ...
        self.last_images = np.zeros((200, ROI[3], ROI[2], N_CHANNELS))
        self.image_number = 0
        self.ep_len = 0
        ...
        if self.save_obs == True:
            if not os.path.isfile("video\\info.txt"):
                with open("video\\info.txt", "a+") as info:
                    info.write("ticks,speed,cte,throttle,steering\n")
                    info.close()    
        ...
        self.viewer = DonkeyUnitySimContoller(level=level, port=33333, max_cte_error=max_cte_error)
    ...
    def postprocessing_step(self, action, observation, reward, done, info):
        ...
        concat_speed = np.zeros((1,1))
        concat_speed[0,0] = info["speed"]
        #observation = np.concatenate((observation, concat_speed), axis=-1)
    ...
    def reset(self):
        ...
        if self.save_for_vae == True:
            for idx, im in enumerate(self.last_images):
                if (idx < self.ep_len) and (self.ep_len < 3000):
                    cv2.imwrite("vae\\train3\\img{:04d}.png".format(self.image_number), im)
                    self.image_number += 1
                self.last_images = np.zeros((200, ROI[3], ROI[2], N_CHANNELS))
    ...
    def observe(self):
        ...
        encoded = observation
        if self.save_for_vae == True:
            self.last_images[1:] = self.last_images[:-1]
            self.last_images[0] = observation
            self.ep_len += 1
        ...
        if self.save_obs == True:
            ...
            with open("video\\info.txt", "a") as inf:
                inf.write("{},{},{},{},{}\n".format(self.n_image,info["speed"],info["cte"],info["throttle"], info["steering"]))
                inf.close()
