...
class CnnSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(CnnSACPolicy, self).__init__(*args, **kwargs,
                                              layers=[32, 16],
                                              act_fun=tf.nn.elu,
                                              feature_extraction="cnn")

class CnnAltSACPolicy(SACPolicy):
    def __init__(self, *args, **kwargs):
        super(CnnAltSACPolicy, self).__init__(*args, **kwargs,
                                              layers=[50],
                                              act_fun=tf.nn.elu,
                                              feature_extraction="cnn")
...
register_policy('CnnSACPolicy', CnnSACPolicy)
register_policy('CnnAltSACPolicy', CnnAltSACPolicy)
...
def make_env(seed=0, log_dir=None, vae=None, frame_skip=None,
    ...
    if log_dir is None and log_dir != '':
        log_dir = "\\tmp\\gym\\{}\\".format(int(time.time()))
...
def create_test_env(stats_path=None, seed=0,
                    log_dir='', hyperparams=None, teleop=False):
    """
    ...
    if not teleop:
        env = DummyVecEnv([make_env(seed, log_dir, vae=vae, teleop=teleop, frame_skip=TEST_FRAME_SKIP)])
    else:
        env = make_env(seed, log_dir, vae=vae, teleop=teleop,
                   n_stack=hyperparams.get('frame_stack', 1), frame_skip=TEST_FRAME_SKIP)()
    ...
    if teleop:
        env = TeleopEnv(env, is_training=False)
...
def create_callback(algo, save_path, verbose=1):
    ...
    def sac_callback(_locals, _globals):
        ...
        if _locals['done'] == True:
            ...
            if _locals['last_ep_len'] >= 3001 and _locals['last_ep_reward'] > 100:
                _locals['self'].save("{}_{:02d}".format(save_path, _locals['episode']))
