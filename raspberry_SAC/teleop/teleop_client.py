...
class TeleopEnv(object):
    ...
    def step(self, action):
        ...
        elif self.well_done_event.is_set():
            done = True
            self.well_done_event.clear()
        else:
            done = False
    ...
    def main_loop(self):
        ...
        last_time_pressed = {'space': 0, 'm': 0, 't': 0, 'b': 0, 'o': 0, 'n': 0}
        ...
        while not end:
            ...
            if keys[K_n] and (time.time() - last_time_pressed['n']) > KEY_MIN_DELAY:
                if self.is_training:
    ...
    def update_screen(self, action):
        ...
        if (self.donkey_env is not None
            ...
            #cv2.imwrite("videos/dec_{:04d}.png".format(self.im_tick), reconstructed_image)
            # Convert BGR to RGB
